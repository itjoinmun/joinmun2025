...
func (r *paymentRepo) GetPaymentByDelegateEmail(delegateEmail string) (*dashboard.Payment, error) {
	var payments dashboard.Payment
	query := `SELECT * FROM payments WHERE mun_delegate_email = $1`
	err := r.db.Select(&payments, query, delegateEmail)
	if err != nil {
		logger.LogError(err, "Failed to get payments for team", map[string]interface{}{"delegateEmail": delegateEmail, "layer": "repository", "operation": "repo.GetPaymentsBydelegateEmail"})
		return nil, err
	}
...
func (r *paymentRepo) UploadPayment(payment *dashboard.Payment) error {
...
func (r *paymentRepo) MakeInitialPayment(tx *sqlx.Tx, payment *dashboard.Payment, delegateEmail string) (int, error) {
	query := `INSERT INTO payments (mun_delegate_email, package, payment_file, payment_status, payment_date, payment_amount) 
